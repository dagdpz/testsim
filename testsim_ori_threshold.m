function testsim_ori_threshold
% try some PTB stimulus presentation

% Modified from:
% http://peterscarfe.com/thresholddemo.html
...
% Open the screen
%***IK: antialiasing!
antialiasing = 1; % multisample
% help AntiAliasing
[window, windowRect] = PsychImaging('OpenWindow', screenNumber, grey, [], 32, 2,[], [antialiasing],  kPsychNeed32BPCFloat);
%                                   ('OpenWindow',screenNumber,color,[rect],[pixelSize],[numberOfBuffers],[stereomode],[multisample],[imagingmode]);
...
for trial = 1:numTrials
	...
	% ***IK - but above is not congruent to key responses, so I flipped it (-theAngle) below in Screen('DrawTextures'...) - 
	% so now negative ccw (left), positive cw (right)
	% BUT for bar, convention is fine - negative is to left (ccw)!
	...
	elseif 1 % ***IK, Draw Bar as in monkeypsych
		BarSizePix1= 50; % deg2pix_withOffset (par.shape.BarSize_L,par.offset_deg);
		BarSizePix2= 10; % deg2pix_withOffset (par.shape.BarSize_W,par.offset_deg);

		rectBar = [BarSizePix2 BarSizePix1 BarSizePix2 BarSizePix1];
		% stimRect_Bar       = aux_pr2rect([par.x  par.y]', rectBar);
		stimRect_Bar       = aux_pr2rect([xCenter yCenter]', rectBar);

		%Screen('FillOval', window, [200,200,200]);
		img = 0.8*ones(20,100);

		texture = Screen('MakeTexture', window, img);
		Screen('DrawTextures',window, texture,[], stimRect_Bar, theAngle);
		
	elseif 0 %***IK, as in http://peterscarfe.com/rotatingsquaresdemo.html
		...
		% Translate, rotate, re-tranlate and then draw
		Screen('glPushMatrix', window)
		Screen('glTranslate', window, xCenter, yCenter)
		Screen('glRotate', window, theAngle, 0, 0);
		 Screen('glTranslate', window,-xCenter,-yCenter)
		Screen('FillRect', window, [0.8 0.8 0.8],...
			CenterRectOnPoint([-BarSizePix2 -BarSizePix1 BarSizePix2 BarSizePix1], xCenter, yCenter));
...
ylabel('Left proportion');
title('Psychometric function');

% Clean up
sca;

% functions from monkeypsych
