function testsim_cosine_fits_LS
...
for n_level = 1:length(noise_levels)
    ...
    for n_bins = 1:length(bin_list)
        ...
        data.R{n_bins,n_level} = noise_levels(n_level) * rand(n_rep,bin_list(n_bins)*n_rpeaks); % use uniformly distributed random data
        data.C{n_bins,n_level} = cos(x-pi);                                          % simulate cosine
        data.vM{n_bins,n_level} = vonMises_mod(1, 0, 4, pi, x);                      % simulate von Mises distribution with theta-hat = pi, kappa = 4
        ...
        data.RC{n_bins,n_level}  = bsxfun(@plus, (max(data.C{n_bins,n_level}) - min(data.C{n_bins,n_level})) * data.R{n_bins,n_level}, data.C{n_bins,n_level}+2); % noisy cosine
        data.RvM{n_bins,n_level} = bsxfun(@plus,(max(data.vM{n_bins,n_level}) - min(data.vM{n_bins,n_level})) * data.R{n_bins,n_level},data.vM{n_bins,n_level});% noisy von Mises distribution
...
%             toc
            ...
            for ii = 1:n_rep
                ...
                k1 = 1; %circ_kappa(x, currCurve' - mean(currCurve) + 0.1592);
                t1 = b;
                d1 = mean(currCurve);
...
%             
            ...
            title(['R^2 Cosfit SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
            ...
            title(['R^2 VMfit SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
            ...
            subplot(length(noise_levels),length(bin_list), length(bin_list)*(n_level-1) + n_bins)
            plot(repmat(x,1,n_rpeaks),data.(dist_types{n_type}){n_bins,n_level}(1,:),'.')
            hold on
            plot(repmat(x,1,n_rpeaks), out.(dist_types{n_type}).cosine.yfit{n_bins,n_level}(1,:))
            ...
            plot(repmat(x,1,n_rpeaks),data.(dist_types{n_type}){n_bins,n_level}(1,:),'.')
            hold on
            plot(repmat(x,1,n_rpeaks), out.(dist_types{n_type}).vonMises.yfit{n_bins,n_level}(1,:))
...
%         title({'Fit Significance of Mosher''s procedure', 'After thresholding by R-squared'})
        ...
        title(['Cosfit sig (R^2) SNR:' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        ...
        title(['VMfit sig (R^2) SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
...
%         
        ...
        title(['Cosfit  R^2 SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        clear Rsq
        ...
        title(['VMfit R^2 SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        clear Rsq
        ...
        title(['Cosfit scaling SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        ...
        title(['Cosfit Theta SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        clear cosPh
                ...
                title(['VMfit scaling SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        ...
        title(['VM Theta SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        clear vmPh
        ...
        title(['VM kappa SNR: ' num2str(noise_levels(n_level)) ' ' num2str(bin_list(n_bins)) 'bins, ' (dist_types{n_type}) ])
        clear vmKappa
