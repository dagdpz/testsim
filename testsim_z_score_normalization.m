...
elseif 0 % one task, two "conditions" (e.g. pre- and post-injection, or two tasks), three epochs: "baseline" "response1" "response2", concatenated trials
...
elseif 1 % one task, two "conditions" (e.g. pre- and post-injection, or two tasks), three epochs: "baseline" "response1" "response2", zscore by baseline
...
concatenate = 0; % std of concatenated trials or use std across mean trial amplitude

b1 = amp1(1) + noise1(1)*randn(n_trials1,n_samples1/3);
s1 = [b1 amp1(2) + noise1(2)*randn(n_trials1,n_samples1/3) amp1(3) + noise1(3)*randn(n_trials1,n_samples1/3)]; % trials x samples
mean_s1 = mean(s1,1);


% zs1 = zscore(reshape(s1,n_trials1*n_samples1,1)); % concatenated full trials

if concatenate
    zs1 = (s1 - mean(reshape(b1,n_trials1*n_samples1/3,1)) ) / std(reshape(b1,n_trials1*n_samples1/3,1)); % std concatenated trials
else
    zs1 = (s1 - mean(mean(b1,2))) / std(mean(b1,2)); % std across mean trial amplitudes
end
...
if ndims(zs1) == 1,
    ...
    mean_zs1 = mean(reshape(zs1,n_trials1*n_samples1,1));
    std_zs1 = std(reshape(zs1,n_trials1*n_samples1,1));
end
    
b2 = amp2(1) + noise2(1)*randn(n_trials2,n_samples2/3);
s2 = [b2 amp2(2) + noise2(2)*randn(n_trials2,n_samples2/3) amp2(3) + noise2(3)*randn(n_trials2,n_samples2/3)]; 
mean_s2 = mean(s2,1);
...
% condition 2 is zscored using mean and std of condition 1!
if concatenate
	zs2 = (s2 - mean(reshape(b1,n_trials1*n_samples1/3,1))) / std(reshape(b1,n_trials1*n_samples1/3,1)); % std concatenated trials
else
    zs2 = (s2 - mean(mean(b1,2))) / std(mean(b1,2)); % std across mean trial amplitudes
end
...
if ndims(zs2) == 1,
    ...
    mean_zs2 = mean(reshape(zs2,n_trials2*n_samples2,1));
    std_zs2 = std(reshape(zs2,n_trials2*n_samples2,1));
end
